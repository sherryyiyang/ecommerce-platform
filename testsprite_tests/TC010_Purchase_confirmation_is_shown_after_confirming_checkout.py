...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /login (http://localhost:5173/login) using the required navigate action.
        ...
        # -> Fill the email with the test account 'example@gmail.com', fill the password with '123456789', then click the Login button to try signing in with the test account shown on the page.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div/div/div/div/div/div/form/div/div/input').nth(0)
